...
typedef struct starlight_image_buffer_t {
    StarlightBuffer b;
    uint32_t w;
    uint32_t h;
    int32_t x;
    int32_t y;
    // uint32_t z; // Zoom
    // float a; // Aspect Ratio: width / height
    // uint32_t zw; // zoom width
    // uint32_t zh; // zoom height
} StarlightImageBuffer;
...
/* scale { */
typedef starlight_status_t (starlight_scale_t)(
    StarlightImageBuffer *, StarlightImageBuffer *, bool
);
starlight_scale_t starlight_scale_bc;
starlight_scale_t starlight_scale_bl;
starlight_scale_t starlight_scale_nn;
/* } */
